...
class Home extends React.Component {
    ...
    componentDidMount() {
        let isMirror = this.props.location.search && this.props.location.search.includes('?mirror')
        this.setState({
        ...
        requestAnimationFrame(this.setClock)
        fetch('https://api.rss2json.com/v1/api.json?rss_url=https://www.firstpost.com/rss/india.xml').then(res => res.json()).then(feed => {
            this.setState({ rssFeed: feed })
        })
    }

    setClock = () => {
        this.setState({ clock: dayjs().format('HH:mm'), seconds: dayjs().format('ss') })
        requestAnimationFrame(this.setClock)
    }
    ...
    onOkNewMember = () => {
        ...
        if (this.props.location.state) {
            ...
            password = localStorage.getItem('playThisPass')
        }
                    ...
                    (!this.state.requested ?
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', marginRight: '1rem' }}>
                                {this.state.isMirror ?
                                    <span><span className="mirrorTime1">{this.state.clock}</span> <span className="mirrorTime2">{this.state.seconds}</span></span>
                                    : <h1 style={{ marginBottom: 0, fontSize: getFontSize(this.state.username.length) }} className={this.state.isMirror ? "titleMirror" : "leftMargin"} > {this.state.username}</h1>}
                                {!this.state.isMirror && <div class="share-playlist">
                            ...
                            {this.state.isMirror && this.state.rssFeed && <News items={this.state.rssFeed && this.state.rssFeed.items} />}
                            <hr />
                            <div className={this.state.isMirror ? 'myrow-mirror' : `myrow`}>
                                <div className={this.state.isMirror ? 'left-center' : `left`}>
                                    <Current
                                        username={this.props.match.params.username}
                                    ...
                                    <Playlist
                                        ...
                                        isMirror={this.state.isMirror}
                                    />
                                    {this.state.isOwner && !this.state.isMirror ? <span className="leftMargin"><Input value={this.state.newVideoUrl} style={{ width: 300, marginBottom: '1rem' }} onChange={e => this.setState({ newVideoUrl: e.target.value })} placeholder="Add a video url" onPressEnter={() => { this.setState({ newVideoUrl: '' }); this.modifyPlaylist('add') }} /></span> : null}
                                </div>
                                {!this.state.isMirror && <div className="right">
